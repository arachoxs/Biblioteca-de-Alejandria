...
import NuevoHiloModal from "@/components/mensajeria/NuevoHiloModal";
import {
  crearHiloAction,
  agregarRespuestaAction,
  getMisHilosAction,
  getHiloDetalleAction,
  getStatsClienteAction,
} from "./actions";
...
export default function MensajeriaPage() {
  ...
  const cargarHilos = useCallback(async () => {
    try {
      const [hilosData, statsData] = await Promise.all([
        getMisHilosAction(1, 10, filtroEstado),
        getStatsClienteAction(),
      ]);
      setHilos(hilosData);
      setStats(statsData);
  ...
  async function handleHiloClick(hiloId: string) {
    try {
      setCargando(true);
      const hilo = await getHiloDetalleAction(hiloId);
      if (hilo) {
  ...
  async function handleCrearHilo(titulo: string, mensaje: string) {
    try {
      setEnviando(true);
      const result = await crearHiloAction(titulo, mensaje);
      if (result.success) {
  ...
  async function handleEnviarRespuesta(mensaje: string) {
    ...
    try {
      setEnviando(true);
      const result = await agregarRespuestaAction(hiloSeleccionado.id, mensaje);
      if (result.success) {
        const hiloActualizado = await getHiloDetalleAction(hiloSeleccionado.id);
        if (hiloActualizado) {
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-brand-accent mt-1">
              Gestiona tus consultas con nuestro equipo
            </p>
          </div>
          <button
            type="button"
            onClick={() => setModalNuevoAbierto(true)}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-brand-primary hover:bg-brand-primary/90 transition-colors cursor-pointer"
          >
        ...
        {cargando ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-brand-accent">
              Crea un nuevo hilo para contactar a nuestro equipo
            </p>
          </div>
        ) : (
