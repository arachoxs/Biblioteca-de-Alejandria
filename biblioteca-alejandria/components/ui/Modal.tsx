...
export default function Modal({ isOpen, onClose, title, children }: ModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-text/50 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div 
        className="relative bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 ring-1 ring-brand-primary/5"
        role="dialog"
        aria-modal="true"
        aria-labelledby="modal-title"
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-brand-accent/10 bg-brand-bg">
          <h2 id="modal-title" className="text-xl font-bold text-brand-primary font-display tracking-tight">
            {title}
          </h2>
          <button
            onClick={onClose}
