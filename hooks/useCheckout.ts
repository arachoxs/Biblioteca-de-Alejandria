...
export type Step = "carrito" | "envio" | "pago" | "confirmacion";
...
export function useCheckoutState() {
  ...
  const goToPago = useCallback((opcion: OpcionEnvio): boolean => {
...
export function useCartSummary(
  cartData: ReservaAgrupadaItem[] | null,
  ...
  return useMemo(() => {
    ...
    const itemCount =
      cartData?.reduce((s, g) => s + g.copias_reservadas, 0) ?? 0;
